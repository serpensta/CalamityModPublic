            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                if (!TailSpawned)
                {
                    ...
                    for (int j = 0; j <= maxLength; j++)
                    {
                        ...
                        int segmentSpawn = NPC.NewNPC(NPC.GetSource_FromAI(), (int)(NPC.position.X + (float)(NPC.width / 2)), (int)(NPC.position.Y + (float)NPC.height), segmentType, NPC.whoAmI);
                        ...
                        NetMessage.SendData(MessageID.SyncNPC, -1, -1, null, segmentSpawn);
                        num2 = segmentSpawn;
                    }
