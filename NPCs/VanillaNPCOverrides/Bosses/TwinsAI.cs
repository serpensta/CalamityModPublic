...
namespace CalamityMod.NPCs.VanillaNPCOverrides.Bosses
{
    public static class TwinsAI
    {
        ...
        public static bool VanillaRetinazerAI(NPC npc, Mod mod)
        {
            ...
            if (Main.rand.NextBool(5))
            {
                int num416 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 5, npc.velocity.X, 2f);
                    ...
                    else if (npc.position.Y + (float)npc.height < Main.player[npc.target].position.Y && num425 < (Main.masterMode ? 720f : Main.expertMode ? 560f : 400f))
                    {
                        ...
                        if (npc.ai[3] >= (float)num428)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                Projectile.NewProjectile(npc.GetSource_FromAI(), vector43.X, vector43.Y, num422, num423, num430, attackDamage_ForProjectiles3, 0f, Main.myPlayer);
                            }
                        }
                    }
                }
            ...
            if (npc.ai[0] == 1f || npc.ai[0] == 2f)
            {
                ...
                if (Main.masterMode && npc.ai[2] >= 0.2f)
                {
                    if (npc.ai[1] % 10f == 0f)
                    {
                        ...
                        {
                            int type = ProjectileID.EyeLaser;
                            int attackDamage_ForProjectiles = npc.GetAttackDamage_ForProjectiles(20f, 19f);
                            ...
                            {
                                Vector2 perturbedSpeed = projectileVelocity.RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (float)(numProj - 1)));
                                Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center + Vector2.Normalize(perturbedSpeed) * 50f, perturbedSpeed, type, attackDamage_ForProjectiles, 0f, Main.myPlayer);
                            }
                        }
                    }
                }
            ...
            if (npc.ai[1] == 0f)
            {
                ...
                float num441 = Main.player[npc.target].Center.Y - 300f - vector45.Y;
                if (NPC.IsMechQueenUp)
                {
                    num438 = 14f;
                    num440 = vector41.X;
                ...
                if (npc.localAI[1] > 180f && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                {
                    ...
                    int num446 = Projectile.NewProjectile(npc.GetSource_FromAI(), vector45.X, vector45.Y, num440, num441, num445, attackDamage_ForProjectiles4, 0f, Main.myPlayer);
                }
            ...
            float num450 = Main.player[npc.target].Center.X + (float)(num447 * 340) - vector46.X;
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (npc.localAI[1] > 60f && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                {
                    ...
                    int num455 = Projectile.NewProjectile(npc.GetSource_FromAI(), vector46.X, vector46.Y, num450, num451, num454, attackDamage_ForProjectiles5, 0f, Main.myPlayer);
                }
            }
        ...
        public static bool VanillaSpazmatismAI(NPC npc, Mod mod)
        {
            ...
            if (Main.rand.NextBool(5))
            {
                int num460 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 5, npc.velocity.X, 2f);
                    ...
                    else
                    {
                        ...
                        if (npc.ai[3] >= 60f)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                int num473 = Projectile.NewProjectile(npc.GetSource_FromAI(), vector49.X, vector49.Y, num466, num467, num472, attackDamage_ForProjectiles6, 0f, Main.myPlayer);
                            }
                        }
                    }
                }
                ...
                if (Main.masterMode && npc.ai[2] >= 0.2f)
                {
                    if (npc.ai[1] % 10f == 0f)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center + Vector2.Normalize(projectileVelocity) * 50f, projectileVelocity, type, attackDamage_ForProjectiles, 0f, Main.myPlayer);
                        }
                    }
                }
                ...
                if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        if (npc.localAI[1] > 8f)
                        {
                            ...
                            int num489 = Projectile.NewProjectile(npc.GetSource_FromAI(), vector51.X, vector51.Y, num483, num484, num488, attackDamage_ForProjectiles7, 0f, Main.myPlayer);
                        }
                    }
                }
