...
namespace CalamityMod.NPCs.SupremeCalamitas
{
    public class SepulcherHead : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            float targetX = Main.npc[CalamityGlobalNPC.SCal].position.X + (Main.player[NPC.target].width / 2);
            float targetY = Main.npc[CalamityGlobalNPC.SCal].position.Y + (Main.player[NPC.target].height / 2);
            ...
            if (targetDist <= 110 && AttackCooldown <= 0)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int k = 0; k < totalProjectiles; k++)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, velocity * 1.2f, type, damage, 0f, Main.myPlayer, 0, 0, 2);
                    }
                    NPC.netUpdate = true;
                }
