            ...
            if (Projectile.owner == Main.myPlayer)
            {
                for (int j = 0; j < 6; j++)
                {
                    if (j % 2 != 1 || Main.rand.NextBool(3))
                    {
                        ...
                        int bee = Projectile.NewProjectile(Projectile.GetSource_FromThis(), projPos.X, projPos.Y, beeVelX, beeVelY, Main.player[Projectile.owner].beeType(), Main.player[Projectile.owner].beeDamage(Projectile.damage / 3), Main.player[Projectile.owner].beeKB(0f), Main.myPlayer);
