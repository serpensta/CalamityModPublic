    ...
    public class WingmanHoldout : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else if (PostFireCooldown <= 0 && (Owner.Calamity().mouseRight || (firingDelay <= 0 && Projectile.ai[2] == 1 || Projectile.ai[2] == -1)))
            {
                ...
                {
                    if ((Owner.CheckMana(Owner.ActiveItem(), heldItem.mana * 5, true, false) && Projectile.ai[2] == 1) || (Projectile.ai[2] == -1 && Owner.statMana > heldItem.mana * 15))
                    {
                        Shoot(heldItem, true);
                        PostFireCooldown = 20 + 65 * Utils.GetLerpValue(10, 40, FiringTime, true);
                        ShootingTimer = 0;
                        FiringTime = 40;
                    }
                    else
